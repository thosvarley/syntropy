...
The goal of Syntropy is to be the Networkx of information theory: lowering the barrier to entry for information theory so that powerful tools are accessible to scientists from many different fields. 
Accordingly, the Syntropy package is written in Python and depends on only a small number of standard scientific-computing libraries: ``numpy``, ``scipy``, and ``networkx``. The optional neural (normalizing-flow) estimators additionally require ``torch`` and ``nflows``. 
...
The Syntropy package is broken up by different classes of estimators (discrete, Gaussian, KNN-based, etc), which can be applied to different kinds of data. 
...
Wherever possible, we have tried to ensure consistency between all branches, ensuring that each information theoretic function has both discrete and continuous implementations (e.g. one can estimate the mutual information using discrete, Gaussian, Kraskov-based, and mixed-datatype estimators, multiple estimators of the Shannon/differentiable entropy, the O-information, etc). 
...
# Example
A short example illustrates the basic workflow for two different estimators on the same real-valued data. A feature of Syntropy is that, in addition to the expected (average) value of a measure, the estimators return the *pointwise* (local) value for every sample, allowing the distribution of an information measure across a dataset to be inspected directly. Below, we compute the mutual information between two correlated Gaussian variables using Gaussian and KNN-based estimators, recovering both the expected value and its local values.

```python
import numpy as np
from syntropy.gaussian import mutual_information, local_mutual_information

rng = np.random.default_rng(0)
num_samples, rho = 100_000, 0.6
x = rng.standard_normal(num_samples)
y = rho * x + np.sqrt(1 - rho**2) * rng.standard_normal(num_samples)
data = np.vstack([x, y])
cov = np.cov(data, ddof=0)

mi_gaussian = mutual_information(idxs_x=(0,), idxs_y=(1,), cov=cov)
lmi_gaussian = local_mutual_information(idxs_x=(0,), idxs_y=(1,), cov=cov, data=data)
# mi_gaussian == 0.223 nats == lmi_gaussian.mean()

# The same analysis, with Kraskov-based estimators
from syntropy.knn import mutual_information
lmi_kraskov, mi_kraskov = mutual_information(
    idxs_x = (0,), 
    idxs_y = (1,),
    data = data, 
    k = 5,
...
# mi_gaussian and mi_kraskov should be close.
print(f"Gaussian MI: {mi_gaussian:.4} nat\nKraskov MI: {mi_kraskov:.4} nat")
```
